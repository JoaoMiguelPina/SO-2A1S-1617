...
#define COMANDO_CREDITAR2 "creditar2"
...
void get_cmd(char *args[], comando_t buffer[], int numargs){ /*Funcao que le comandos e coloca-os no buffer*/
	...
	else if (!strcmp(args[0], COMANDO_CREDITAR2)) {
		new_cmd.operacao = OP_CREDITAR2;
		new_cmd.idConta1 = atoi(args[1]);
		new_cmd.idConta2 = atoi(args[2]);
		new_cmd.valor = atoi(args[3]);
	}
...
void* processCmd(void *cmd_buffer){ /*Funcao que e passada a pthread_create*/
	...
	while(1){
		...
		else if(cmd.operacao == OP_CREDITAR2){
			if (creditar2 (cmd.idConta1, cmd.idConta2, cmd.valor) < 0){
				printf("%s(%d, %d, %d): Erro\n\n", COMANDO_CREDITAR2, cmd.idConta1, cmd.idConta2, cmd.valor);
			}
			else{
				printf("%s(%d, %d, %d): OK\n\n", COMANDO_CREDITAR2, cmd.idConta1, cmd.idConta2, cmd.valor);
			}
...
int main (int argc, char** argv) {
	...
	while (1) {
		...
		/* Creditar2 */
		else if (strcmp(args[0], COMANDO_CREDITAR2) == 0) {
			if (numargs < 4) {
				printf("%s: Sintaxe inválida, tente de novo.\n\n", COMANDO_CREDITAR2);
				continue;
			}
